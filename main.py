...
"""==================================================================================================="""
save_path_for_local = opt.save_path + '/'+opt.dataset + '/' + opt.group
if not os.path.exists(save_path_for_local):
    os.mkdir(save_path_for_local)
...
def save_network(network, epoch_label):
    save_filename = 's%s_net_%s.pth' % (str(opt.seed), epoch_label)
    save_path_ = os.path.join(save_path_for_local, save_filename)
    torch.save(network.cpu().state_dict(), save_path_)
...
# Load model
#---------------------------
def load_network(network, epoch_label):
    save_filename = 's%s_net_%s.pth' % (str(opt.seed), epoch_label)
    save_path_ = os.path.join(save_path_for_local,save_filename)
    network.load_state_dict(torch.load(save_path_))
    return network
...
### teacher model ###
teacher_model      = archs.select(opt.arch, opt)

_  = teacher_model.to(opt.device)
...
iter_count = 0
loss_args  = {'batch':None, 'labels':None, 'batch_features':None, 'f_embed':None, 'teacher_batch':None,
              'epoch':None, 'kd_alpha':None, 'kd_tau':None, 'kd_beta':None, 'n_epochs':None}
...
for epoch in range(opt.n_epochs):
    ...
    ### load teacher model from last epoch
    if epoch > 0:
        teacher_model = load_network(teacher_model, str(epoch - 1))
    ...
    for i,out in enumerate(data_iterator):
        ...
        with torch.no_grad():
            teacher_embeds = teacher_model(**model_args)
        if isinstance(embeds, tuple): embeds, (avg_features, features) = embeds
        if isinstance(teacher_embeds, tuple): teacher_embeds, (teacher_avg_features, teacher_features) = teacher_embeds

        ### Compute Loss
        loss_args['batch']          = embeds
        loss_args['labels']         = class_labels
        loss_args['teacher_batch']  = teacher_embeds
        loss_args['epoch']          = epoch
        loss_args['n_epochs'] = opt.n_epochs
        ...
        if i==len(dataloaders['training'])-1: data_iterator.set_description('Epoch (Train) {0}: Mean Loss [{1:.4f}]'
                                                                            .format(epoch, np.mean(loss_collect)))
