...
class Diffusion(object):
    """Diffusion class
    """
    def __init__(self, features, opt):
    ...
    def get_offline_results(self, n_trunc, kd=50):
        ...
        global trunc_ids, lap_alpha, trunc_init
        if self.use_ann:
            _, trunc_ids = self.ann.search(self.features, n_trunc)
            sims, ids = self.knn.search(self.features, kd)
            lap_alpha = self.get_laplacian(sims, ids, self.opt.diffusion_w)
        else:
        ...
        print('[offline] 2) prepare random walk matrix')
        RW_matrix = []
        for i in range(self.N):
            trunc_init_vec = cp.zeros((self.N, 1))
            trunc_init_vec[i,0] = 1
            RW_matrix_vec = (1 - self.opt.diffusion_w) * linalg.cg(lap_alpha, trunc_init_vec)

            RW_matrix.append(RW_matrix_vec)
    ...
    def get_laplacian(self, sims, ids, alpha=0.99):
        ...
        degrees = affinity @ cp.ones(num) + 1e-12
        # mat: degree matrix ^ (-1/2)
        mat = sparse.dia_matrix(
            (degrees ** (-0.5), [0]), shape=(num, num), dtype=cp.float32)
        stochastic = mat @ affinity @ mat
        sparse_eye = sparse.dia_matrix(
            (cp.ones(num), [0]), shape=(num, num), dtype=cp.float32)
    ...
    def get_affinity(self, sims, ids, gamma=3):
        ...
        vec_ids = cp.asarray(vec_ids)
        mut_ids = cp.asarray(mut_ids)
        mut_sims = cp.asarray(mut_sims)

        affinity = sparse.coo_matrix((mut_sims, (vec_ids, mut_ids)),
                                     shape=(num, num), dtype=cp.float32)
        return affinity
