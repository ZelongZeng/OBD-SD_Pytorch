...
def basic_training_parameters(parser):
    ...
    parser.add_argument('--evaluation_metrics', nargs='+',
                        default=['e_recall@1', 'e_recall@2', 'e_recall@10', 'nmi', 'f1', 'mAP_1000', 'mAP_lim', 'mAP_c', \
                                 'dists@intra', 'dists@inter', 'dists@intra_over_inter', 'rho_spectrum@0', \
                                 'rho_spectrum@-1', 'rho_spectrum@1', 'rho_spectrum@2', 'rho_spectrum@10'], type=str,
...
def wandb_parameters(parser):
    ...
    parser.add_argument('--wandb_key',       default='Your wandb key',  type=str,   help='API key for W&B.')
...
##########################################
def LSD_training_parameters(parser):
    ### Parameters for the Loss of Self-distillation
    parser.add_argument('--PSD', action='store_true',
                        help='If set, use PSD.')
    parser.add_argument('--OBDP', action='store_true',
                        help='If set, use OBDP.')

    parser.add_argument('--kd_tau', default=1.0, type=float, help='The temperature ``tau'' of knowledge distillation.')
    parser.add_argument('--kd_lambda', default=75.0, type=float, help='The weight of knowledge distillation.')
    parser.add_argument('--diffusion_w', default=0.99, type=float, help='The probability of random walk for the diffusion process.')
    parser.add_argument('--diffusion_tau', default=1.0, type=float, help='The temperature ``tau" for the softmax-based normalization.')
    return parser

def noisy_label_parameters(parser):
    ### Parameters for the Generation of Noisy data
    parser.add_argument('--noisy_label', action='store_true',
                        help='If set, use noisy labels.')
    parser.add_argument('--noisy_ratio', default=0.0, type=float, help='noisy ratio.')
    return parser

def load_checkpoint(parser):
    ### Parameters for the Loading of Trained Models
    parser.add_argument('--starting_checkpoint_epoch', default=10, type=int, help='starting_checkpoint_epoch.')
    parser.add_argument('--checkpoint_name', default='checkpoint_name', type=str, help='checkpoint_name.')
    return parser
